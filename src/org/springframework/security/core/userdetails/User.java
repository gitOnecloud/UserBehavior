...
public class User implements UserDetails, CredentialsContainer {
    ...
    public Collection<GrantedAuthority> getAuthorities() {
    	if(this.authorities == null) {//延迟读取用户角色等信息
    		...
    		cn.onecloud.model.cmdb.menu.User user = SecuManager.findUserR_ByU_a(username);
			//用户id
			this.id = new int[] {user.getId()};
			//用户真实姓名、头像
			this.str = new String[] {user.getName()};
			//用户角色
	        ...
	        if(!rids.isEmpty())
	        	userMenu.addAll(SecuManager.findMenuByRIds(rids.substring(0, rids.length()-1)));
