...
function index_createTab(title, href, load, newtitle) {
	$.get(href + "?_=" + Math.random(), function(data) {
		...
		if(load) {
			...
			$.ajax({
				url: "script/onecloud/" + url.split("_")[0] + "/" + url + ".js",
...
$(function() {
	...
	//进入首页默认打开第一个页面
	//$('#index_treeDemo li:first a:first').trigger('click');
});
var index_panel = [];
/**
 * 首页布局
...
function loadIndex(reload) {
	...
	if(index_user.id == "") {
		...
		$("#index_user").html("欢迎&nbsp;" + index_user.name + "&nbsp;");
	}
}
/**
 * 打开登录系统框
 ...
 * 格式化输出流量
 */
function index_formatTraffic(traffic, unit) {
	if(traffic == 0) return '0B';
	if(unit == null) {
		unit = 1;
	}
	if(traffic >= 1099511627776)
		return (traffic/1099511627776).toFixed(unit) + "TB";
	else if(traffic >= 1073741824)
		return (traffic/1073741824).toFixed(unit) + "GB";
	else if(traffic >= 1048576)
		return (traffic/1048576).toFixed(unit) + "MB";
	else
		return (traffic/1024).toFixed(unit) + "KB";
}
/**
 * 格式化输出浏览数
 */
function index_formatPage(page) {
	if(page >= 100000000)
		return (page/100000000).toFixed(1) + "亿次";
	else if(page >= 10000000)
		return (page/10000000).toFixed(1) + "千万次";
	else if(page >= 1000000)
		return (page/1000000).toFixed(1) + "百万次";
	else if(page >= 10000)
		return (page/10000).toFixed(1) + "万次";
	else
		return page + "次";
}
/**
 * 格式化输出统计数
 */
function index_formatNum(page) {
	if(page >= 100000000)
		return (page/100000000).toFixed(1) + "亿个";
	else if(page >= 10000000)
		return (page/10000000).toFixed(1) + "千万个";
	else if(page >= 1000000)
		return (page/1000000).toFixed(1) + "百万个";
	else if(page >= 10000)
		return (page/10000).toFixed(1) + "万个";
	else
		return page + "个";
}
/**
 * 格式化输出费用
 */
function index_formatFee(fee) {
	if(fee == 0) return '0元';
	fee /= 1000;
	if(fee >= 1000000)
		return (fee/1000000).toFixed(2) + "百万元";
	else if(fee >= 10000)
		return (fee/10000).toFixed(2) + "万元";
	else
 ...
 * 格式化输出时间
 */
function index_formatHour(hour) {
	if(hour == 0) return '0小时';
	return (hour/1000/60/60).toFixed(3) + "小时";
}
/**
 * 对数据中的某一项进行冒泡排序
 * @param data 排序数据
 * @param sort 排序位
 */
function index_sortData(data, sort) {
	var temp;
	var exchange;
	for(var i=0; i<data.length; i++) {
		exchange = false;
		for( var j=data.length-2; j>=i; j--) {
			if((data[j+1][sort]) >
					(data[j][sort])) {
				temp = data[j+1];
				data[j+1] = data[j];
