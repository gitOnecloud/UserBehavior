	...
	$.each(opts.queryParams, function(k, v) {
		switch(v.type) {
			case 'text':
				...
				break;
			case 'combo':
				_53a[k] = $("#" + v.id).combo('getValue');
				break;
			case 'combobox':
				_53a[k] = $("#" + v.id).combobox('getValue');
				break;
			case 'datebox':
				_53a[k] =  $("#" + v.id).datebox('getValue');
				break;
			case 'datetimebox':
				_53a[k] = $("#" + v.id).datetimebox('getValue');
				break;
			case 'numberbox':
				_53a[k] =  $("#" + v.id).numberbox('getValue');
				break;
			case 'combotree':
				_53a[k] = $("#" + v.id).combotree('getValue');
				break;
			case 'combogrid':
				_53a[k] = $("#" + v.id).combogrid('getValue');
				break;
			default:
				_53a[k] = v;
		}
...
if(opts.pagination){
	_53a[opts.paramsName.page] = opts.pageNumber;
	_53a[opts.paramsName.num] = opts.pageSize;
}
//end
if(opts.sortName){
...
//update 增加跳转到最后一页的功能
},loadLast:function(jq,_58f){
	return jq.each(function(){
	var opts=$(this).datagrid("options");
	var pager=$(this).datagrid("getPager");
	opts.pageNumber = Math.ceil(
			$.data(pager[0],"pagination").options.total/opts.pageSize);
	pager.pagination({pageNumber:opts.pageNumber});
	_537(this,_58f);
	});
//end
...
if(opts.mode=="remote"){
	//update 更改远程读取时发送的参数名
	var remoteParam = {};
	remoteParam[opts.remotePName] = q;
	_770(_775,null, remoteParam,true);
	//end
}else{
...
var remoteParam = {};
remoteParam[opts.remotePName] = q;
grid.datagrid("load",$.extend({},opts.queryParams,remoteParam));
//end
}else{
if(!q){
return;
